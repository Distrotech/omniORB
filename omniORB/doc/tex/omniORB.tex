...
{\Large \bf Changes and Additions, April 2012}
\begin{itemize}
\item Updates for omniORB 4.2.
\end{itemize}
...
for making sense of multi-threaded code, but it adds overhead to the
logging so it can be disabled.
...
overhead, so it can be turned off.
...
\code{CORBA::TRANSIENT}. If this parameter is set true, omniORB
...
therefore, prevents the use of recursive TypeCodes).
...
triggered for every incoming call\footnote{Except that with the
  \code{threadPoolWatchConnection} parameter set true, a thread can
  perform multiple upcalls even when thread pool mode is active.}. As
with the \code{createThread} interceptor, the interceptor function
must call the \type{info\_T}'s \op{run} method to pass control to the
upcall.
